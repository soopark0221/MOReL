...
def flatten(params):
    tmp = [i.contiguous().view(-1, 1) for i in params]
    out = torch.cat(tmp).view(-1) # in tensor
    outlist = out.detach().cpu().numpy() # in numpy
    return out
...
def unflatten_like(vector, likeTensorList):
    ...
    for tensor in likeTensorList: 
        ...
        n = tensor.numel()
        s = tensor.shape()
        outtensor.append(vector[i:i+n].view(s)) # in tensor
